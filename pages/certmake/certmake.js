...
Page({
  ...
  bindaddress: function () {
    if (this.data.fixed_address_flag == false){
      ...
      wx.redirectTo({
        url: '../leasing/leasing?detail_id=' + this.data.detail_id + '&type=' + this.data.type + '&count=' + this.data.goods_count
      })
    }
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (app.globalData.leasing_id){
          wx.request({
            url: 'https://www.hattonstar.com/getFixedAddress',
            data: {
